...
def login(user: User, response: Response):
    ...
    return {"message": "Connection successful", "user_id": user.username}
...
async def upload_file(file: UploadFile = File(...), session_id: Optional[str] = Cookie(None)):
    ...
    try:
        ...
        return {"message": f"File {filename} loaded and cleaned", "pages": len(docs), "path": file_path}
...
def doc_classify(session_id: Optional[str] = Cookie(None)):
    ...
    # We call doc_summary internal logic or use the result
...
def doc_translate(mode: str = "summary", session_id: Optional[str] = Cookie(None)):
    ...
    summary_res = doc_summary(session_id)
    text_to_translate = summary_res["summary"]
...
def run_agent(input: AgentInput, session_id: Optional[str] = Cookie(None)):
    ...
    try:
        ...
        # Here we simplify by using the loaded DOC_STORE as context
        if not tools.DOC_STORE:
            raise HTTPException(status_code=400, detail="No document loaded for searching.")
        ...
        memory.add_ai_message(str(response_text.content))
